...
var version = "0.6.0"
var targetPath = "target/chaosblade-0.6.0/bin"

var osSpecFile = path.Join(targetPath, fmt.Sprintf("chaosblade-os-spec-%s.yaml", version))
var k8sSpecFile = path.Join(targetPath, fmt.Sprintf("chaosblade-k8s-spec-%s.yaml", version))
var dockerSpecFile = path.Join(targetPath, fmt.Sprintf("chaosblade-docker-spec-%s.yaml", version))

var jvmSpecFile = path.Join(targetPath, fmt.Sprintf("chaosblade-jvm-spec-%s.yaml", version))
var cplusSpecFile = path.Join(targetPath, "chaosblade-cplus-spec.yaml")
var nodeSpecFile = path.Join(targetPath, fmt.Sprintf("chaosblade-docker-spec-%s.yaml", version))
...
func main() {
	...
	models := mergeModels(osModels, dockerModels, k8sModels, jvmModels, cplusModels)
